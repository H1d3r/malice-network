...
- `args` [table<string>] -  string array of arguments
...
- `session` [Session] -  special session
- `bundle` [string] -  bundle name to unload

**Example**

```
unload_module(active(),"bundle_name")
```
...
- `data` [string] -  raw bytes as string
- `key` [string] -  XOR key

**Example**

```
xor_bytes(data, "mykey")
```
...
Send a natural-language task to the active chat backend

Chat sends a natural-language message to the active chat backend.
...
```
chat(active(), "List files in the current directory")
```
...
Stream real-time LLM interaction events from the agent session

Tapping activates real-time monitoring of an LLM agent session.
...
- `sess` [Session] - special session
- `enable` [boolean] - enable or disable keepalive (true/false)
