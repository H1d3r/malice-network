...
`exec`、`shell`、`powershell` 这类命令很多都是异步回传结果。

- 使用 `-f/--file` 保存输出时，client 会先等待任务完成，再把所有已回传的输出分片汇总写入文件。
- 这意味着 `-f` 适合保存完整结果，不需要再额外依赖 `--wait` 来保证文件内容完整。
- 如果命令本身启用了静默模式或目标侧没有返回输出，生成的文件可能为空。
